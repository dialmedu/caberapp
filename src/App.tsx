  ...
  return (
    <div className="min-h-screen bg-slate-100 p-8 flex flex-col items-center">
      ...
      <div className="mt-8 flex gap-2 no-print">
        <button onClick={() => window.print()} className="btn-compact bg-slate-900 text-white px-8 py-2.5 font-black uppercase">Imprimir</button>
        <button onClick={onBack} className="btn-compact border px-8 py-2.5 font-black uppercase">Regresar</button>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const filteredData = useMemo(() => {
    const base = data[view === 'visits' ? 'visits' : view] || [];
    let result = [...base];
    if (searchQuery) {
    ...
    <div className="h-screen w-full flex overflow-hidden bg-slate-100 font-sans text-slate-900 selection:bg-indigo-100">
      <aside className={`${isMenuCollapsed ? 'w-12' : 'w-48'} bg-slate-900 text-white flex flex-col transition-all duration-200 border-r border-slate-800 shrink-0`}>
        <div className="p-3 flex items-center justify-between border-b border-white/5">
          {!isMenuCollapsed && <span className="font-black text-[10px] uppercase truncate opacity-60 tracking-widest leading-none">{data.settings.appName}</span>}
        ...
        <header className="h-12 bg-white border-b border-slate-200 px-6 flex items-center justify-between shrink-0 sticky top-0 z-40 shadow-sm">
          ...
          <div className="flex gap-2">
             <div className="relative flex items-center mr-2">
                ...
                <input type="text" placeholder="Búsqueda..." value={searchQuery} onChange={(e)=>setSearchQuery(e.target.value)} className="pl-6 pr-2 py-1 border rounded bg-slate-50 text-[9px] w-32 outline-none focus:border-indigo-400 focus:bg-white" />
             </div>
             {view === 'inventory' && userRole === 'admin' && <button onClick={() => setModal({ type: 'inventory_crud' })} className="btn-compact bg-slate-900 text-white"><Plus size={12}/> Equipo</button>}
          ...
          {view === 'visits' && (
            <div className="space-y-4">
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                  {paginatedData.filter((v:any)=>v.date === selectedDate).map((visit: any) => {
                    ...
                    return (
                      <div key={visit.id} className={`card-base p-3 flex flex-col gap-3 transition-all ${visit.status === 'finished' ? 'opacity-50 grayscale bg-slate-50' : 'hover:border-indigo-400 bg-white'}`}>
                        ...
                        <div className="flex-1">
                          <p className="font-black text-[11px] truncate mb-0.5 leading-tight uppercase">{person?.name || 'Cargando...'}</p>
                        ...
                        {visit.status === 'active' && (
                          <button onClick={() => handleFinishVisit(visit.id)} className="w-full bg-indigo-600 text-white py-1.5 rounded text-[9px] font-black uppercase tracking-widest hover:bg-red-600 transition-colors">Finalizar</button>
                        )}
                      </div>
                    );
                  })}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-lg w-full max-w-sm shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border border-slate-300">
            ...
            <div className="p-5 overflow-y-auto custom-scrollbar flex-1 bg-white">
               ...
               {/* Resto de flujos omitidos para brevedad pero siguen la misma lógica estructural */}
            </div>
          </div>
        </div>
      )}
